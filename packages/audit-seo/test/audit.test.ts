...
describe("audit-seo", () => {
  ...
  it("projects a low-scoring LHR with issues for failing audits", async () => {
    ...
    expect(result.status).toBe("success")
    if (result.status === "success") {
      expect(result.issues.some((i) => i.rule === "seo/document-title")).toBe(true)
      expect(result.issues.some((i) => i.rule === "seo/meta-description")).toBe(true)
      expect(result.issues.some((i) => i.rule === "seo/is-crawlable")).toBe(true)
    }
  })
})
