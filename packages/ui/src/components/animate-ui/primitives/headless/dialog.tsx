...
type DialogBackdropProps<TTag extends React.ElementType = typeof motion.div> = Omit<
...
function DialogBackdrop<TTag extends React.ElementType = typeof motion.div>(
  ...
  const { as = motion.div, transition = { duration: 0.2, ease: "easeInOut" }, ...rest } = props
...
function DialogClose<TTag extends React.ElementType = "button">(props: DialogCloseProps<TTag>) {
  ...
  return <CloseButton data-slot="dialog-close" as={as as React.ElementType} {...rest} />
}
...
function DialogTitle<TTag extends React.ElementType = "h2">(props: DialogTitleProps<TTag>) {
