...
function Effect({
  ...
  if (slide) {
    const offset =
      typeof slide === "boolean" ? DEFAULT_SLIDE_OFFSET : (slide.offset ?? DEFAULT_SLIDE_OFFSET)
    const direction =
      typeof slide === "boolean"
        ? DEFAULT_SLIDE_DIRECTION
        : (slide.direction ?? DEFAULT_SLIDE_DIRECTION)
    const axis = direction === "up" || direction === "down" ? "y" : "x"
    hiddenVariant[axis] = direction === "right" || direction === "down" ? -offset : offset
  ...
  if (fade) {
    ...
    visibleVariant.opacity =
      typeof fade === "boolean" ? DEFAULT_FADE_OPACITY : (fade.opacity ?? DEFAULT_FADE_OPACITY)
  }
  ...
  if (zoom) {
    ...
    visibleVariant.scale =
      typeof zoom === "boolean" ? DEFAULT_ZOOM_SCALE : (zoom.scale ?? DEFAULT_ZOOM_SCALE)
  }
...
function Effects({ children, delay = 0, holdDelay = 0, ...props }: EffectsProps) {
